create or replace PACKAGE BODY "XXBBNA_WAREHOUSE_PROCESS_PKG" AS
  ...
  --  Description   : Package built for re-designed Yard Management App to interact with Oracle DB           #
  --                                                                                                         #
  ...
  ------------------------------------------------------------------------------------------------------|    #
  --  |08/31/26 |Anthony Vauthier   |1.0     |Created xxbbna_warehouse_process_pkg program 1.0 Version 
  --                                         |Notes: This package body was built referencing existing
  --                                                 package                                                 #
  ----#-------------------------------------------------------------------------------------------------|    #
  -- #########################################################################################################
  ...
  BEGIN
    ...
    FROM   (SELECT scac_code,
            ...
            --and
            -- ORDER  BY 3, 2 ASC) x;
            ORDER BY name_order, carrier_name;
  EXCEPTION
    WHEN others THEN
      dbms_output.put_line('Exception ------ ' || sqlerrm);
      RAISE;
  ...
  --
  ----------------------------------------------------------------------------------------------------------------------
  PROCEDURE xxbbna_warehouse_org_code(x_org_cur OUT SYS_REFCURSOR) IS
  BEGIN
  ...
  --
  ----------------------------------------------------------------------------------------------------------------------
  PROCEDURE xxbbna_warehouse_truck_id(x_org_code IN VARCHAR2, x_truck_id_cur OUT SYS_REFCURSOR) IS
    --
  ...
  BEGIN
    ...
    INTO   l_is_valid
    FROM   (SELECT 'Y'
            FROM   oe_order_headers_all oh, mtl_parameters ood
            WHERE  1 = 1
            AND    oh.org_id = ood.organization_id
...
PROCEDURE xxbbna_loading_shipping_proc_m(p_org          IN VARCHAR2,
  ...
  PROCEDURE xxbbna_update_truck_id(p_order_number       NUMBER,
                                   ...
                                   p_truck_flag         VARCHAR2, --(TO UPDATE CUSTOM TABLE WITH M-MANUAL,S-SCAN)
  ...
  BEGIN
    ...
    INTO   l_user_id
    FROM   fnd_user fu, fnd_user pp
    WHERE  fu.employee_id = pp.employee_id
...
PROCEDURE xxbbna_truck_manifest_proc(p_organization_code IN VARCHAR2,
  ...
  --
  ----------------------------------------------------------------------------------------------------------------------
  PROCEDURE xxbbna_product_type(x_product_type_cur OUT SYS_REFCURSOR) IS
    --
    -- Local variable declaration
    --
  ...
  --
  ----------------------------------------------------------------------------------------------------------------------
  PROCEDURE xxbbna_cat_product_type_rel(x_category_product_type_cur OUT SYS_REFCURSOR) IS
    --
    -- Local variable declaration
    --
  ...
  BEGIN
    ...
    OPEN x_category_product_type_cur FOR
  ...
  --
  ----------------------------------------------------------------------------------------------------------------------
  PROCEDURE xxbbna_product_type_questions(x_product_type_question_cur OUT SYS_REFCURSOR) IS
    --
    -- Local variable declaration
    --
  ...
  BEGIN
    ...
    OPEN x_product_type_question_cur FOR
  ...
  --
  ----------------------------------------------------------------------------------------------------------------------
  PROCEDURE xxbbna_product_type_answers(x_product_type_answers_cur OUT SYS_REFCURSOR) IS
    --
    -- Local variable declaration
    --
  ...
  BEGIN
    ...
    OPEN x_product_type_answers_cur FOR
  ...
  ----------------------------------------------------------------------------------------------------------------------------
  PROCEDURE xxbbna_single_point_org_list(p_org_code IN VARCHAR, x_org_list_cur OUT SYS_REFCURSOR) IS
    --
    -- Local variable declaration
    --
    -- l_org_list orglist;
  ...
  ----------------------------------------------------------------------------------------------------------------------------
  PROCEDURE xxbbna_ip_addr_list(x_ip_addr_cur OUT SYS_REFCURSOR) IS
    -- l_ip_addr ipaddr;
  BEGIN
    -- BULK   COLLECT
    -- INTO   l_ip_addr
    OPEN x_ip_addr_cur FOR
  ...
  BEGIN
    ...
    BEGIN
      UPDATE xxwsh_container_loading xcl
      SET    staged_truck_id  = DECODE(p_stagged_flag, 'Y', p_header_truck, NULL) 
            ,
