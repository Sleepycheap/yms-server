...
export async function getAllContainersForOrder(orderNumber) {
  try {
    const result = await connection.execute(
      ...
      `SELECT order_no, cont_name, total_cont_qty AS cont_qty, total_gross_wt AS cont_gross_wt, ship_from_org_code, truck_id_2, ship_set_name, item_description, organization_id FROM (SELECT a.order_no, a.cont_name, SUM(a.cont_qty) OVER (partition by a.cont_name) AS total_cont_qty, SUM(a.cont_gross_wt) OVER (partition by a.cont_name) AS total_gross_wt, a.ship_from_org_code, a.truck_id_2, a.ship_set_name, b.item_description, b.organization_id, ROW_NUMBER() OVER (PARTITION BY a.cont_name ORDER BY a.cont_name ASC) as rn FROM xxwsh_containers a, wsh_delivery_details b WHERE a.delivery_detail_id = b.delivery_detail_id AND a.order_no = :ordernumber ) WHERE rn = 1`,
      [orderNumber],
    );
    const { rows } = result;
    return rows;
